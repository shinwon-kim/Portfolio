...
const Home = (): JSX.Element => {
  ...
  useGSAP(() => {
    ...
    if (containerRef.current) {
      ...
      // gsap.to( arrowRef.current, {y: 1, repeat: -1, yoyo: true, duration: 0.8, ease: "power1.inOut"})
      ...
      >
        <h1 className="text-3xl mt-5 font-bold">
          ...
          <span className="relative inline-flex" ref={nameRef}>
            ...
            <span
              ref={ballRef}
              className="absolute w-4 h-4 opacity-0 bg-[#0061FF] rounded-full bottom-10 left-0 z-10"
            />
          </span>
          입니다
        </h1>
        ...
        <p className="mt-3 p-2 text-center font-semibold break-keep whitespace-pre-line">무언가를 만드는 것이 좋아 프로그래밍을 시작했으며, 사용자가 직접 마주하는 화면을 설계하고 구현하는 과정에 흥미를 느끼게 되었습니다. <br/>
        직관적인 디자인과 뛰어난 사용자 경험을 고민하며, 최고의 UI/UX를 실현하는 프론트엔드 개발자가 목표입니다.</p>
